...
Scaffolded: 2026-09-16 from seed `Template_Positions` Data!EFC  
...
| Field | Value | Lock |
|-------|-------|------|
| Symbol | EFC | book-cited |
| Underlying / sleeve (seed) | Mortgage REIT | book-cited |
...
| Provider (seed) | Ellington | book-cited |
| Frequency (seed) | Monthly | book-cited — confirm if live differs |
| Risk tier (seed) | Foundation | book-cited (Position Details) |
| Div type (seed) | DIV-1 | book-cited |
| ROC scope | REIT (confirm from 1099) | confirm on recreate if not CASH/MLP |

## Book SoR (cite seed / SQLite only; markdown does not write DB)

| Field | Value | Notes |
|-------|-------|-------|
| In-year ROC estimate (seed) | 25% | Book SoR via seed/`position_characteristic`; change only via Accept / `roc_pct_change` |
...
| Seed notes (abbrev) | Fill roc_pct from 1099 / year-end tax docs | REIT — confirm from 1099 | 2026 EST 25.0% — Mortgage REIT — ROC possible when dist > taxable income. No clean 2026 19a-1. Planning 25%. | Context only |
...
1. Before asking about ROC / issuer / frequency / scope / amounts for EFC, read this file (+ related Authority packs).
