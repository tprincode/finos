...
Scaffolded: 2026-09-16 from seed `Template_Positions` Data!GLAD  
...
| Field | Value | Lock |
|-------|-------|------|
| Symbol | GLAD | book-cited |
| Underlying / sleeve (seed) | BDC | book-cited |
...
| Provider (seed) | Gladstone | book-cited |
...
| ROC scope | BDC (1099; confirm character) | confirm on recreate if not CASH/MLP |

## Book SoR (cite seed / SQLite only; markdown does not write DB)

| Field | Value | Notes |
|-------|-------|-------|
| In-year ROC estimate (seed) | 5% | Book SoR via seed/`position_characteristic`; change only via Accept / `roc_pct_change` |
...
| Seed notes (abbrev) | Fill roc_pct from 1099 / year-end tax docs | BDC — confirm from 1099 | 2026 EST 5.0% — BDC — distributions typically ordinary/interest; low ROC. Planning 5%. | Context only |
...
1. Before asking about ROC / issuer / frequency / scope / amounts for GLAD, read this file (+ related Authority packs).
