...
#[tokio::test]
async fn backtest_run_does_not_post_dividend_facts() {
    ...
    let body = must_ok(
        ...
        "BacktestRun",
        serde_json::json!({
            "scenario": "vxus-rebalance",
            "hypotheticalPnlMinor": 12_500,
            "scale": 2,
    ...
    assert_eq!(body["runs"][0]["scenario"].as_str().unwrap(), "vxus-rebalance");
    assert_eq!(
        body["runs"][0]["hypotheticalPnlMinor"].as_i64().unwrap(),
        12_500
    );

    let got = query_json(&platform, "BacktestGet").await;
    assert_eq!(got["runs"][0]["scenario"].as_str().unwrap(), "vxus-rebalance");
