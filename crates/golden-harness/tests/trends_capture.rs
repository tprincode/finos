...
async fn count_cash_adjust(platform: &LocalPlatform, period_end: &str) -> usize {
    ...
    acts.as_array()
        .unwrap()
        .iter()
        .filter(|a| a["activityType"] == "Cash_Adjust" && a["occurredOn"] == period_end)
        .count()
}
...
#[tokio::test]
async fn blank_cash_skips_update_and_does_not_adjust() {
    let root = repo_root();
    let production = root.join("database/seed/production");
    let dir = tempfile::tempdir().unwrap();
    let platform = LocalPlatform::open(dir.path().join("app-data")).await.unwrap();
    ...
    .await;
    let stored = query_json(
        &platform,
        "TrendsWeekGet",
        Some(r#"{"asOfDate":"2026-08-28"}"#),
    )
    .await;
    assert_eq!(stored["carCashMinor"], cash[4]);
    let skipped = serde_json::json!({
        "periodStart": "2026-08-22",
        "periodEnd": "2026-08-28",
        "capturedAt": "2026-08-29T13:00:00Z",
        "incomeBalanceMinor": 20_000_000,
        "rothBalanceMinor": 1_000_000,
        "speculationBalanceMinor": 2_000_000,
        "healthBalanceMinor": 1_500_000,
        "carBalanceMinor": 4_100_000,
        "acct9BalanceMinor": 3_500_000,
        "incomeCashMinor": null,
        "rothCashMinor": null,
        "speculationCashMinor": null,
        "healthCashMinor": null,
        "carCashMinor": null,
        "acct9CashMinor": null,
        "acct9EtfValueMinor": 0,
        "scale": 2,
    ...
    must_cmd(&platform, "WeekCaptureAccept", skipped).await;
    assert_eq!(count_cash_adjust(&platform, "2026-08-28").await, 0);
    ...
    assert_eq!(after["carBalanceMinor"], 4_100_000);
    assert_eq!(
        after["carCashMinor"], cash[4],
        "blank Car cash on Accept must skip; keep the last typed cash"
    );
}
