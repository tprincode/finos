...
#[tokio::test]
async fn distribution_characterize_does_not_post_dividend_or_rewrite_oracles() {
    ...
    let activity_id = posted["activityId"]
        .as_str()
        .expect("DividendActualRecord returns activityId");
    let body = must_ok(
        &platform,
    ...
    .await;
    assert_eq!(body["characterizations"].as_array().unwrap().len(), 1);
    assert_eq!(
        body["characterizations"][0]["category"].as_str().unwrap(),
        "return_of_capital"
    );
    assert_eq!(
        body["characterizations"][0]["amountMinor"].as_i64().unwrap(),
        10_000
    );

    let got = query_json(&platform, "DistributionGet").await;
    assert_eq!(got["characterizations"].as_array().unwrap().len(), 1);
    assert_eq!(
        got["characterizations"][0]["activityId"].as_str().unwrap(),
        activity_id
    );
