...
pub async fn week_capture_accept(
    ...
    let existing_week = canonical.trends_week_get(record.period_end.clone()).await?;
    if let Some(existing) = existing_week.as_ref() {
    ...
    let accounts = canonical.account_list().await?;
    if let Some(ex) = existing_week.as_ref() {
        if let Some(inc) = accounts.iter().find(|a| a.name == "Income") {
            if !typed_cash.iter().any(|(id, _)| *id == inc.account_id) {
                record.income_cash_minor = ex.income_cash_minor;
            }
        }
        if let Some(a9) = accounts.iter().find(|a| a.name == "9") {
            if !typed_cash.iter().any(|(id, _)| *id == a9.account_id) {
                record.acct9_cash_minor = ex.acct9_cash_minor;
            }
        }
    }
